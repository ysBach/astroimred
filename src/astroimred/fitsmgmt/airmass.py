...
def airmass_to_hdr(
    ...
    _t = Time.now()
    if obscoord is None:
        ra = as_quantity(header.get(ra) if isinstance(ra, str) else ra, u.deg)
        de = as_quantity(header.get(dec) if isinstance(dec, str) else dec, u.deg)
    ...
    expt = as_quantity(header[exptime] if isinstance(exptime, str) else exptime, u.s)
