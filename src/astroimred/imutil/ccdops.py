...
from .._core.numeric import binning
from .._core.types import FITSLike, FQArr, HDUExt
from ..fitsmgmt import header as headers
from ..fitsmgmt import io as _io
from ..fitsmgmt.io import inputs2list
...
def cut_ccd(
    ccd: CCDData,
    position: tuple[float, float] | list[float] | np.ndarray | SkyCoord,
    size: int | float | tuple[int, int] | tuple[float, float] | u.Quantity,
    wcs: WCS | None = None,
    mode: str = "trim",
    fill_value: float = np.nan,
    warnings: bool = True,
...
def bin_ccd(
    ccd: CCDData,
    factors: int | tuple[int | None, ...] | list[int | None] | None = None,
    ...
    factors : `list`-like of `int`, optional.
        ...
        ``numeric.binning(..., order_xyz=True)``. For 2-D data this is
