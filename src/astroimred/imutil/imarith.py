...
from astroimred._core.types import FITSLike, HDUExt, StrPathLike
from astroimred.fitsmgmt.header import cmt2hdr, update_tlm
from astroimred.fitsmgmt.io import _has_header, _parse_extension, _parse_image
from astroimred.imutil.ccdops import CCDData_astype, imslice
from astroimred.logging import logger

from ._util_fits import _get_image_hdu
...
def _update_hdr(
    ...
    if isinstance(params, str):
        ...
        raise TypeError(
            "hdr_params must be None, str, or dict of header keyword-value pairs."
        )
...
def _load_im_name_hdr(
    ...
    extension1,
    extension2,
    offsets: str | np.ndarray | list[float] | tuple[float, ...] | None = None,
    force_ccddata: bool = False,
    verbose: bool = False,
):
    """Prepare images as `~numpy.ndarray` unless FORCING to become `~astropy.nddata.CCDData`.
    ...
    if (
        offsets is not None
    ):  # We always have to open headers of both, numeric or ndarray not acceptible.
        im1, im1name, _ = _parse_image(
            im1, extension1, name1, prefer_ccddata=True
        )  # Returns CCDData only if
        im2, im2name, _ = _parse_image(
            im2, extension2, name2, prefer_ccddata=True
        ...
        im1, im1name, _ = _parse_image(
            im1, extension1, name1, force_ccddata=force_ccddata
        )
        im2, im2name, _ = _parse_image(
            im2, extension2, name2, force_ccddata=force_ccddata
        )
...
def imarith(
    im1: FITSLike | int | float | np.number,
    op: str,
    im2: FITSLike | int | float | np.number,
    ...
    if ignore_header:  # Only work with ndarray
        ...
        if error_calc:  # Only work with CCDData
            ...
            res = CCDData_astype(
                eval(f"im1.{operator}(im2)"), dtype=dtype
            )  # in CCDData
