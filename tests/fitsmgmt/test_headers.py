...
class TestHeaderExports:
    ...
    def test_header_helpers_live_in_headers(self):
        """Header edit/accessor helpers live in header."""
...
class TestHdrval:
    ...
    def test_priority_units_and_source(self):
        ...
        assert header.hdrval(None, hdr, "EXPTIME", default=0) == 20
        assert header.hdrval(3, hdr, "EXPTIME", default=0) == 3
        assert header.hdrval(None, hdr, "MISSING", default=0) == 0
        assert header.hdrval(3 * u.s, hdr, "EXPTIME", unit=u.s) == 3 * u.s

        value, source = header.hdrval(
...
class TestCmt2Hdr:
    ...
    def test_add_history(self, sample_header):
        ...
        header.cmt2hdr(hdr, "h", "Test history entry", time_fmt=None)
    ...
    def test_add_comment(self, sample_header):
        ...
        hdr = sample_header.copy()
        header.cmt2hdr(hdr, "c", "Test comment entry", time_fmt=None)
    ...
    def test_history_aliases(self, sample_header, histcomm):
        ...
        hdr = sample_header.copy()
        header.cmt2hdr(hdr, histcomm, "Test", time_fmt=None)
        assert "HISTORY" in hdr
    ...
    def test_invalid_histcomm_raises(self, sample_header):
        ...
        with pytest.raises(ValueError):
            header.cmt2hdr(hdr, "invalid", "Test", time_fmt=None)
...
class TestUpdateProcess:
    ...
    def test_add_process(self, sample_header):
        ...
        hdr = sample_header.copy()
        header.update_process(hdr, process="B")
    ...
    def test_append_process(self, sample_header):
        ...
        hdr = sample_header.copy()
        hdr["PROCESS"] = "B"
        header.update_process(hdr, process="D")
